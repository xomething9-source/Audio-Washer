...
**Audio Washer**는 인공지능(AI) 생성 음원 및 가이드 레코딩의 지저분한 요소를 제거하고, 상업적 스트리밍 표준에 맞게 자동으로 마스터링해주는 **100% 브라우저 기반 오디오 프로세싱 툴**입니다.

특히 **Suno AI**와 같은 서비스로 생성된 음원 특유의 과도한 잔향과 탁한 중저역대를 정돈하여, 즉시 배포 가능한 수준(70점 이상의 품질)으로 끌어올리는 데 최적화되어 있습니다.
...
*   **AI De-esser**: 귀를 찌르는 치찰음(ㅅ, ㅊ 등)을 실시간 분석하여 부드럽게 제어합니다.
...
1.  **빌드 및 실행**
    ...
    ```
2.  **사용 방법**
    1.  `Load Audio` 버튼을 클릭하거나 파일을 드래그하여 불러옵니다.
    2.  `Play`를 눌러 Real-time으로 변화하는 사운드를 확인합니다.
    3.  `Dry/Wet` 스위치로 마스터링 전후를 비교합니다.
    4.  결과가 만족스러우면 `WAV` 또는 `MP3`를 선택하고 `Export`를 클릭합니다.
